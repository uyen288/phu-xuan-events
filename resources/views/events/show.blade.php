...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            {{-- Category Badge --}}
            <span class="inline-block bg-red-100 text-red-700 text-xs font-semibold px-3 py-1 rounded-full mb-3">
            ...
            @if($event->tags->isNotEmpty())
                <div class="flex flex-wrap gap-2 mb-6">
                    @foreach($event->tags as $tag)
                        <span class="bg-gray-100 text-gray-700 text-xs px-3 py-1 rounded-full border">
                            #{{ $tag->name }}
                        </span>
                    @endforeach
                </div>
                    ...
                    @if(auth()->user()->isStudent())
                        ...
                        @else
                            ...
                            <form action="{{ route('registrations.store', $event->id) }}" method="POST">
                                ...
                                <button type="submit"
                                        class="w-full bg-red-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-red-800 transition">
                                    Đăng ký tham gia
                                </button>
                            </form>
                        @endif
